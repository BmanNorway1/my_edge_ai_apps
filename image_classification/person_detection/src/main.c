...
/* Converts a chunk of RGB565 pixels to quantized int8 and writes them into the
   model input buffer, padding the 128x128 camera image into the 160x128 model frame. */
...
/* Logs each detected bounding box with its coordinates, head name, and confidence score. */
...
/* Initialises camera, LEDs, Axon platform and model, then runs person detection every 500ms. */
int main(void)
{
	int err;
