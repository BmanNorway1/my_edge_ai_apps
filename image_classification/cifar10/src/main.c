...
#include <generated/nrf_axon_model_cifar10_model_.h> 
...
#define SCALE_FACTOR (CAM_WIDTH / MODEL_WIDTH)

#define NUM_CLASSES 10 // CIFAR-10 has 10 classes
#define SCORE_THRESHOLD 90

#define FRAME_RGB565_BYTES ((CAM_WIDTH) * (CAM_HEIGHT) * 2)

static const char * const cifar10_labels[] = {
    "airplane", "automobile", "bird", "cat", "deer",
    "dog", "frog", "horse", "ship", "truck"
};

static uint16_t cam_frame_buf[CAM_WIDTH * CAM_HEIGHT]; /* rå RGB565, hele kamerabildet */
static int8_t lut_8bit[256];
...
/* Downsamples the 96x96 RGB565 frame to 32x32 via 3x3 box averaging,
   then quantizes into the model input buffer (HWC layout, matching training data). */
static void downsample_and_quantize(void)
{
    for (size_t out_row = 0; out_row < MODEL_HEIGHT; out_row++) {
        for (size_t out_col = 0; out_col < MODEL_WIDTH; out_col++) {
            uint32_t r_sum = 0, g_sum = 0, b_sum = 0;

            for (size_t dy = 0; dy < SCALE_FACTOR; dy++) {
                for (size_t dx = 0; dx < SCALE_FACTOR; dx++) {
                    const size_t cam_row = out_row * SCALE_FACTOR + dy;
                    const size_t cam_col = out_col * SCALE_FACTOR + dx;
                    const uint16_t pixel = cam_frame_buf[cam_row * CAM_WIDTH + cam_col];

                    /* Expand RGB565 to 8-bit-per-channel before averaging */
                    r_sum += ((pixel >> 11) & 0x1f) << 3;
                    g_sum += ((pixel >> 5)  & 0x3f) << 2;
                    b_sum += (pixel & 0x1f) << 3;
                }
            }

            const size_t n = SCALE_FACTOR * SCALE_FACTOR;
            const uint8_t r_avg = (uint8_t)(r_sum / n);
            const uint8_t g_avg = (uint8_t)(g_sum / n);
            const uint8_t b_avg = (uint8_t)(b_sum / n);

            /* HWC layout: interleaved per pixel, matching training data (NHWC) */
            const size_t dst = (out_row * MODEL_WIDTH + out_col) * 3;

            input_buf[dst + 0] = lut_8bit[r_avg];
            input_buf[dst + 1] = lut_8bit[g_avg];
            input_buf[dst + 2] = lut_8bit[b_avg];
        }
    }
}

/* Converts a raw 0-255 pixel intensity to a quantized int8 using the model's scale and zero-point. */
...
/* Precomputes a quantized int8 lookup table for raw 8-bit pixel intensities (0-255),
   shared across R/G/B channels. */
static void prefill_luts(const nrf_axon_nn_compiled_model_input_s *in)
{
    for (int i = 0; i < 256; i++) {
        lut_8bit[i] = quantize((float)i, in);
    }
}
...
/* Copies a chunk of raw RGB565 pixels from the video buffer into the full-frame buffer. */
static void convert_chunk_to_cam_buf(const uint8_t *chunk_buf, size_t pixel_start, size_t pixel_count)
{
    for (size_t p = 0; p < pixel_count; p++) {
        cam_frame_buf[pixel_start + p] = extract_pixel(chunk_buf, p);
    }
}

/* Streams and reassembles a full RGB565 frame from the camera into cam_frame_buf. */
static int capture_one_frame(const struct device *video)
{
	...
	while (total < FRAME_RGB565_BYTES) {
        ...
        convert_chunk_to_cam_buf(vbuf->buffer, total / 2, chunk / 2);
...
static int capture_and_detect(const struct device *video, const nrf_axon_nn_compiled_model_s *model)
{
	...
	for (int i = 0; i < NUM_CLASSES; i++) {
		LOG_INF("Class %s: score %d", cifar10_labels[i], output_buf[i]);
	}

	return 0;
}

/* Initialises camera, LEDs, Axon platform and model, then runs cifar10 classification every 500ms. */
int main(void)
{
	int err;

	nrf_axon_result_e result;
	const nrf_axon_nn_compiled_model_s *model = &model_cifar10_model;
