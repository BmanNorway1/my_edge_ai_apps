...
#include "nrf_axon_model_finger_digits_v2_4_.h"  // your compiled model
...
static const char * const finger_digit_labels[] = {
    "Five", "Four", "One", "Unknown", "Three", "Two", "Zero"
};

/* Model now takes single-channel (grayscale) input -- retrained/recompiled
 * with input_shape=(H, W, 1) instead of (H, W, 3), since the source images
 * are black & white anyway. This cuts input_buf to 1/3 its previous size. */
...
/* Computed once at startup from the model's real quant params -- not hardcoded.
 * This way, retraining/recompiling the model with different quantization
 * automatically produces the correct binary values, no manual recalculation needed.
 */
static int8_t binary_white;
static int8_t binary_black;

static void init_binary_values(const nrf_axon_nn_compiled_model_input_s *in)
{
    binary_white = quantize(255.0f, in);  // raw pixel domain: white = 255
    binary_black = quantize(0.0f, in);    // raw pixel domain: black = 0
    LOG_INF("Binary thresholds: white=%d black=%d", binary_white, binary_black);
}
...
static void apply_threshold_to_input_buf(const uint8_t threshold)
{
    ...
    for (size_t i = 0; i < count; i++) {
        input_buf[i] = (gray_buf[i] > threshold) ? binary_white : binary_black;
    }
}
/*==============================================================================*/
                                         ...
                                         const size_t pixel_count)
{
    for (size_t p = 0; p < pixel_count; p++) {
        ...
        const size_t dst_offset = pixel_idx;
...
int main(void)
{
    ...
    const nrf_axon_nn_compiled_model_s *model = &model_finger_digits_v2_4;
    ...
    init_binary_values(model_inputs);
