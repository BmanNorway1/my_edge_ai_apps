...
#include <nrf_axon_model_coffee_cup_vs_banana_9_.h>  // your compiled model
...
static const char *coffee_or_banana[] = {
    "Banana", "Coffee cup", "Unknown"
};
...
static void prefill_luts(const nrf_axon_nn_compiled_model_input_s *in)
{
    for (size_t i = 0; i < ARRAY_SIZE(lut_red_blue); i++) {
        const float value = (float)i / 31.f; // 0.0 to 1.0 - NO symmetry conversion
        lut_red_blue[i] = quantize(value, in);
    }

    for (size_t i = 0; i < ARRAY_SIZE(lut_green); i++) {
        const float value = (float)i / 63.f; // 0.0 to 1.0 - NO symmetry conversion
        lut_green[i] = quantize(value, in);
    }
}
                                         ...
                                         const size_t pixel_count)
{
    for (size_t p = 0; p < pixel_count; p++) {
        ...
        const size_t model_row = PAD_TOP + cam_row;
        const size_t model_col = PAD_LEFT + cam_col;
        const size_t dst_offset = (model_row * MODEL_WIDTH + model_col) * 3; // HWC
        ...
        input_buf[dst_offset + 0] = lut_red_blue[r5]; // R
        input_buf[dst_offset + 1] = lut_green[g6];    // G
        input_buf[dst_offset + 2] = lut_red_blue[b5]; // B
    }
}
    */
                                ...
                                const nrf_axon_nn_compiled_model_s *model)
{
    ...
    for (int i = 0; i < WARMUP_FRAMES; i++) {
        ...
        if (err) {
            LOG_ERR("Warmup frame %d failed (err %d)", i, err);
            (void)video_stream_stop(video, VIDEO_BUF_TYPE_OUTPUT);
            return -1;
        }
    }

    // Capture the actual frame for inference
    err = capture_one_frame(video);
    if (err) {
    ...
    LOG_INF("Raw scores - banana: %d, coffee: %d, unknown: %d",
        output_buf[0], output_buf[1], output_buf[2]);
    ...
    LOG_INF("Detected: %s (score %d)", coffee_or_banana[max_idx], max_val);
    (void)gpio_pin_set_dt(&led_detection, max_idx != 2 ? 1 : 0);
...
int main(void)
{
    ...
    const nrf_axon_nn_compiled_model_s *model = &model_coffee_cup_vs_banana_9;
